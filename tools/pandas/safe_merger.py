...
class SafeMerger(object):
    ...
    def __init__(self, df, keys_dtypes, na_allowed=False):
        self.df = df
        self.set_keys_dtypes(keys_dtypes, na_allowed)

    @staticmethod
    def _make_na_allowed(na_allowed, keys):
        """Make a `na_allowed` of the form key_column -> is_na_allowed (bool)."""
        if isinstance(na_allowed, bool):
            na_allowed = {key: na_allowed for key in keys}
    ...
    def set_keys_dtypes(self, keys_dtypes, na_allowed=False):
        """Set the key columns and the expected dtypes on which df will be merged.
        ...
        self.na_allowed = self._make_na_allowed(na_allowed, self._keys)
        self._check_key_columns(self.df, self._keys, self.dtypes, self.na_allowed)
        return self

    def _check_get_right_left_keys(self, other, on, left_on, right_on):
        ...
        if right_on is not None:
            assert on is None, "on must not be specified if right_on is."
            right_keys = right_on if isinstance(right_on, list) else [right_on]
            if left_on is None and len(right_keys) == 1:
                left_keys = self._keys
            else:
                assert left_on is not None, \
                    "If there are two or more right key columns, " \
                    "left_on must be specified together."
                left_keys = left_on if isinstance(left_on, list) else [left_on]
        else:
            assert left_on is None, "left_on must not be specified if right_on is not."
            if on is not None:
            ...
            left_keys = on
            right_keys = on
        assert len(right_keys) == len(
            left_keys), "left_on and right_on must have the same length: %s vs %s" % (
            left_keys, right_keys)
        wrong_left_keys = list(set(left_keys) - set(self.dtypes.keys()))
        assert len(
            wrong_left_keys) == 0, "These left keys are not set in dtypes: %s" % wrong_left_keys
    ...
    def merge(self, other, how="inner", on=None, left_on=None, right_on=None, na_allowed=False,
        ...
        right_na_allowed = self._make_na_allowed(na_allowed, right_keys)
